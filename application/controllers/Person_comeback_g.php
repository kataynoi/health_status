...
class Person_comeback_g extends CI_Controller
{
    public $user_id;
    public function __construct()
    {
        ...
        $this->load->model('Person_comeback_g_model', 'crud');
    }
    ...
    public function index()
    {
        ...
        $this->layout->view('person_comeback_g/index', $data);
    }
    ...
    function fetch_person_comeback_g()
    {
        ...
        foreach ($fetch_data as $row) {
            ...
            $sub_array[] = $row->note;
           // $sub_array[] = $row->person_comeback_g;
    ...
    public function del_person_comeback_g()
    {
        $id = $this->input->post('id');

        $rs = $this->crud->del_person_comeback_g($id);
        if ($rs) {
            $json = '{"success": true}';
        } else {
            $json = '{"success": false}';
    ...
    public function  save_person_comeback_g()
    {
        $data = $this->input->post('items');
        if ($data['action'] == 'insert') {
            $rs = $this->crud->save_person_comeback_g($data);
            if ($rs) {
            ...
            $rs = $this->crud->update_person_comeback_g($data);
            if ($rs) {
                $json = '{"success": true}';
            } else {
                $json = '{"success": false}';
    ...
    {
        $rs = $this->crud->get_person_comeback_g($id);
        return $rs;
    }

    public function add_person_comeback_g($id=null)
    {
        $data['person']='';
        $data['action']= 'insert';
        if($id!=null){
            $data['person'] = $this->get_person_comeback_g($id);
            $data['action']='update';
        ...
        $this->layout->view('person_comeback_g/add_person_comeback_g', $data);
    }
