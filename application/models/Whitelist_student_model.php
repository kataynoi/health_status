 ...
 */
class Whitelist_student_model extends CI_Model
{
    var $table = "whitelist_student";
    ...
    /* End Datatable*/
    public function del_whitelist_student($id)
    {
        $rs = $this->db
            ->where('id', $id)
            ->delete('whitelist_student');
        return $rs;
    }



    public function save_whitelist_student($data)
    {
        $birth= to_mysql_date($data["birth"]);
            ...
            ->insert('whitelist_student');
            

        return $rs;
    }
    
    public function update_whitelist_student($data)
    {
        $rs = $this->db
    ...
    public function get_whitelist_student($id)
    {
        $rs = $this->db
            ->where('id', $id)
            ->get("whitelist_student")
            ->row();
        return $rs;
    }
    ...
    public function check_student_cid($cid)
    {
        $rs = $this->db
            ->from("whitelist_student")
            ->where('cid', $cid)
            ->count_all_results();
        return $rs;
    }
    public function check_student_age($cid)
    {
        $rs = $this->db
            ->from("t_student_cid")
            ->where('cid', $cid)
    ...
    public function get_student_cid($cid)
    {
        $rs = $this->db
            ->where('cid', $cid)
            ->limit(1)
